...
{
    defineTypeNameAndDebug(aniScalarTransport, 0);
...
Foam::volScalarField& Foam::functionObjects::aniScalarTransport::transportedField()
{
...
Foam::tmp<Foam::volTensorField> Foam::functionObjects::aniScalarTransport::D
(
...
) const
{
    ...
    if (constantD_)
    {
        ...
        (
            ...
            mesh_,
            dimensionedTensor(Dname, phi.dimensions()/dimLength, tensor::I*D_)
        );
    }

    if (nutName_ != "none")
    {
        const volTensorField& nutMean =
            mesh_.lookupObject<volTensorField>(nutName_);

        return tmp<volTensorField>::New(Dname, nutMean);
    }
        ...
        if (turb)
        {
            ...
            (
                ...
                (
                    ...
                    (alphaD_ * turb->mu() + alphaDt_ * turb->mut()) * tensor::I
                )
            );
        }
    }
...
Foam::functionObjects::aniScalarTransport::aniScalarTransport
(
    const word& name,
    const Time& runTime,
...
bool Foam::functionObjects::aniScalarTransport::read(const dictionary& dict)
{
...
bool Foam::functionObjects::aniScalarTransport::execute()
{
    volScalarField& s = transportedField();
...
bool Foam::functionObjects::aniScalarTransport::write()
{
